...
const STATUS_FN   = { pago: [C.green, "PAGO"], recebimento: [C.green, "RECEBIDO"], pendente: [C.orange, "PENDENTE"] };
const STATUS_PRE  = { presente: [C.green, "PRESENTE"], falta: [C.red, "FALTA"], meio_dia: [C.orange, "MEIO DIA"] };
...
function Dashboard({ db }) {
  ...
  const totalOrc = (db.obras || []).reduce((s, o) => s + (o.orcamento || 0), 0);
  const totalGasto = (db.obras || []).reduce((s, o) => s + (o.gasto || 0), 0);
  const ativos = (db.trabalhadores || []).filter(t => t.ativo).length;
  const presencaHoje = (db.presenca || []).filter(p => p.data === window.getTodayBR?.() || p.data === "2026-05-04");
...
function Presenca({ db }) {
  ...
  const hoje = window.getTodayBR?.() || "2026-05-04";
  const presencaHoje = (db.presenca || []).filter(p => p.data === hoje);
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        ...
        <StatBox label="Total" value={presencaHoje.length} color={C.text} />
      </div>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        <thead>
          <tr style={{ borderBottom: `1px solid ${C.border}`, color: C.dim, textAlign: "left" }}>
...
function resolveCommand(raw, db) {
  const parts = raw.trim().split(/\s+/);
  const cmd = parts[0].toLowerCase();
  const args = parts.slice(1);

  if (cmd === "dashboard")     return <Dashboard db={db} />;
  if (cmd === "obras")         return <Obras db={db} args={args} />;
  if (cmd === "trabalhadores") return <Trabalhadores db={db} args={args} />;
  if (cmd === "presenca")      return <Presenca db={db} />;
  if (cmd === "tarefas")       return <Tarefas db={db} args={args} />;
  if (cmd === "estoque")       return <Estoque db={db} args={args} />;
  if (cmd === "financeiro")    return <Financeiro db={db} args={args} />;
  if (cmd === "medicoes")    return <div><Ruler label="MEDIÇÕES" /><div style={{color:C.dim}}>Módulo em desenvolvimento</div></div>;
  if (cmd === "almocos")     return <div><Ruler label="ALMOÇOS" /><div style={{color:C.dim}}>Módulo em desenvolvimento</div></div>;
  if (cmd === "relatorios")    return <Relatorios db={db} />;
...
const BOOT = [
  ...
  { text: "Firebase Realtime DB · Dados Reais", color: C.dim, size: 11 },
...
export default function ConsoleApp() {
  const { db, loading } = useRealtimeDB();
  ...
  const tarefasCriticas = db?.tarefas?.filter(t => t.prioridade === "critica" && t.status !== "concluida").length || 0;
