---
title: obra-etapas.js
description: Mapeador semântico de linguagem natural para etapas de obra padronizadas - usado pela IA para preencher automaticamente campos de tarefa
url: local-file
...
// Mapeador semântico: linguagem natural → etapa de obra padronizada
// Usado pela IA para preencher automaticamente os campos de tarefa

/**
 * LISTA MESTRA DE CORRELAÇÃO
 * 
 * Estrutura:
 *   etapa:      nome padronizado da etapa (usado nos campos da tabela)
 *   codigo:     código interno da etapa
 *   palavras:   termos que indicam essa etapa (linguagem de canteiro)
 * 
 * Adicionar novos termos aqui conforme o uso real identificar lacunas.
 */
const ETAPAS = [
  {
  ...
  },
  {
    ...
    palavras: [
      ...
      'registro', 'registros', 'v��lvula', 'válvulas',
...
/**
 * Detecta etapas mencionadas em um texto livre
 * Retorna array com todas as etapas encontradas (pode ser mais de uma)
 * 
 * @param {string} texto — texto transcrito do áudio ou digitado
 * @returns {Array} — etapas detectadas com score de confiança
 */
  ...
  for (const etapa of ETAPAS) {
    ...
    for (const palavra of etapa.palavras) {
      ...
      if (textoNorm.includes(palavraNorm)) {
        score += palavraNorm.split(' ').length; // termos compostos valem mais
...
/**
 * Extrai tarefas estruturadas de um texto livre usando detecção de etapas
 * 
 * @param {string} texto — transcrição do áudio ou texto do WhatsApp
 * @returns {Object} — tarefas estruturadas prontas para salvar no Firebase
 */
function extrairTarefas(texto) {
  // Quebra o texto em sentenças/itens
  ...
  for (const sentenca of sentencas) {
    ...
    if (etapasDetectadas.length > 0) {
      const melhorEtapa = etapasDetectadas[0]; // maior score
      ...
      tarefas.push({
        ...
        criadaVia:        'whatsapp',
        // prazo e responsavel serão extraídos pelo parser.js
      });
    } else {
      // Sentença sem etapa detectada — entra como tarefa genérica para revisão
      if (sentenca.split(' ').length >= 2) {
        tarefas.push({
          ...
          etapaCodigo: null,
          status:      'aberta',
          confianca:   'baixa', // sinaliza para revisar
          criadaVia:   'whatsapp'
        });
      }
    }
